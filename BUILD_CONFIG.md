...
本项目支持在打包时配置登录页面是否登录需要验证码。通过不同的构建命令,可以生成不同版本的生产环境代码。
...
此命令会构建**登录需要验证码**的生产版本。登录时会显示验证码输入框,并调用 `login` 接口。
...
此命令会构建**不登录需要验证码**的生产版本。登录时不会显示验证码输入框,并调用 `loginByPassword` 接口。
